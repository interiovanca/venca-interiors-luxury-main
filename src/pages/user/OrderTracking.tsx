  ...
  return (
    <div className="min-h-screen bg-background font-sans pb-20">
      ...
      <div className="max-w-4xl mx-auto px-6 -mt-6 relative z-20 space-y-8">
        ...
        {/* Progress Tracker */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card rounded-[2rem] p-8 lg:p-12 shadow-xl border border-border">
          <h3 className="text-xl font-display font-bold text-[#2A2520] mb-10">Delivery Status</h3>

          <div className="relative">
            <div className="absolute left-[27px] top-4 bottom-4 w-1 bg-muted rounded-full z-0" />
            ...
            <div className="space-y-12 relative z-10">
              {steps.map((step, idx) => (
                <div key={idx} className="flex gap-6 items-start">
                  <div className={`w-14 h-14 rounded-2xl flex items-center justify-center shrink-0 shadow-md transition-colors duration-500 ${step.completed ? 'bg-amber-500 text-white shadow-amber-500/30' : 'bg-card text-[#A79D93] border-2 border-border'}`}>
                    {step.icon}
                  </div>
                  <div className="pt-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="bg-card rounded-[2rem] p-8 shadow-md border border-border">
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.3 }} className="bg-card rounded-[2rem] p-8 shadow-md border border-border flex flex-col justify-between">
            <div>
            ...
            <Button className="w-full mt-6 bg-muted hover:bg-[#EAE4D7] text-[#2A2520] rounded-xl h-12 shadow-sm font-semibold flex items-center justify-center gap-2">
